...
// ^ A) Provides HTML and CSS data to webview to display properly formatted page content
// ^ B) Provides icon for switching of starred status
// ^ C) Triggers reload of starred repository when popped
// ^ D) Providers popups and alternative content if no connection or no date received
...
class _RepoDetailPageState extends ConsumerState<RepoDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    // Provides no connection popup if user loses connection
    ...
    return WillPopScope(
      // reload the starred repos list if the user has changed starred status
      onWillPop: () async {
          ...
          loadSuccess: (_) {
            if (_.repoDetail.entity == null) {
              return const NoResultsDisplay(
                message:
                    'These are approximately all the results we could find about this repo right now',
              );
            } else {
              return WebView(
